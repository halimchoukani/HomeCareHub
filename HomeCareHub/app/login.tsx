...
export default function Login() {
  ...
  const toggleVisibility = () => {
    Animated.sequence([
      Animated.timing(iconAnim, { toValue: 0.75, duration: 80, useNativeDriver: true }),
      Animated.timing(iconAnim, { toValue: 1, duration: 120, useNativeDriver: true }),
    ]).start();
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.card, isDesktop && { maxWidth: 450, alignSelf: 'center', width: '100%' }]}>
          ...
          <View style={styles.inputGroup}>
            <View style={styles.wrapper}>
              <Text style={[styles.label, isFocused && styles.labelFocused]}>Mot de passe</Text>
              <Animated.View style={[styles.inputContainer, { borderColor }]}>
                <TextInput
                  ...
                  autoCorrect={false}
                />
                <TouchableOpacity style={styles.iconButton} onPress={toggleVisibility} activeOpacity={0.7} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                  <Animated.View style={{ transform: [{ scale: iconAnim }] }}>
                    {showPassword ? <EyeOpenIcon color={isFocused ? '#5B5FEF' : '#8B8FA8'} /> : <EyeClosedIcon color={isFocused ? '#5B5FEF' : '#8B8FA8'} />}
                  </Animated.View>
                </TouchableOpacity>
              </Animated.View>
              {password.length > 0 && (
                <View style={styles.strengthRow}>
                  {[1, 2, 3, 4].map((i) => (
                    <View key={i} style={[styles.strengthBar, { backgroundColor: password.length >= i * 3 ? (i <= 1 ? '#EF5B5B' : i === 2 ? '#EFA45B' : i === 3 ? '#5BEF8A' : '#5B5FEF') : '#2A2D3E' }]} />
                  ))}
                  <Text style={styles.strengthLabel}>
                    {password.length < 3 ? 'Très faible' : password.length < 6 ? 'Faible' : password.length < 9 ? 'Fort' : 'Très fort'}
                  </Text>
                </View>
              )}
            </View>
          ...
          <TouchableOpacity style={styles.forgotWrapper} onPress={() => router.push('/forgot-password')}>
...
const styles = StyleSheet.create({
  ...
  logoCircle: {
    width: 80, height: 80, borderRadius: 40, backgroundColor: '#1E1B3A', borderWidth: 2, borderColor: '#7C3AED',
    alignItems: 'center', justifyContent: 'center', marginBottom: 12,
    shadowColor: '#7C3AED', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.5, shadowRadius: 12, elevation: 8,
  },
  logoEmoji: { fontSize: 36 },
  appName: { fontSize: 22, fontWeight: 'bold', color: '#FFFFFF', letterSpacing: 1 },
  card: {
    backgroundColor: '#13132A', borderRadius: 24, padding: 28, borderWidth: 1, borderColor: '#2A2750',
  ...
  wrapper: {},
  label: { fontSize: 13, fontWeight: '600', letterSpacing: 0.8, color: '#A0A8C8', marginBottom: 8 },
  ...
  inputContainer: { flexDirection: 'row', width: '100%', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#0D0D1A', borderWidth: 1.5, borderRadius: 12, paddingHorizontal: 16, height: 52 },
  iconButton: { paddingLeft: 12, justifyContent: 'center', alignItems: 'center' },
  strengthRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 8 },
  strengthBar: { flex: 1, height: 3, borderRadius: 2 },
  strengthLabel: { fontSize: 10, fontWeight: '600', letterSpacing: 0.8, color: '#4A4E6A', marginLeft: 6, width: 60, textAlign: 'right' },
  forgotWrapper: { alignSelf: 'flex-end', marginBottom: 24, marginTop: -6 },
  forgotText: { color: '#A78BFA', fontSize: 13, fontWeight: '500' },
  button: { backgroundColor: '#7C3AED', padding: 16, borderRadius: 14, alignItems: 'center', shadowColor: '#7C3AED', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 6 },

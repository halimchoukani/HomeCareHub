...
const MOCK_DATA: Personne[] = [
  { id: '1', name: 'Houda Ajmi', role: 'Famille', statut: 'autorise', image: 'https://randomuser.me/api/portraits/women/44.jpg', heure: '08:14', date: "Aujourd'hui" },
  { id: '2', name: 'Amine Toki', role: 'Médecin', statut: 'autorise', image: 'https://randomuser.me/api/portraits/men/32.jpg', heure: '10:30', date: "Aujourd'hui" },
];

export default function ControleAcces() {
  const router = useRouter();
  const [persons, setPersons] = useState<Personne[]>(MOCK_DATA);
  ...
  const toggleStatut = (id: string, newStatut: Personne['statut']) => {
  ...
  const renderItem = ({ item }: { item: Personne }) => (
    <TouchableOpacity
      style={[styles.card, item.statut === 'bloque' && styles.cardBloque, isDesktop && { width: '24%' }]}
      onPress={() => setSelected(item)}
      activeOpacity={0.85}
    >
      <Image source={{ uri: item.image }} style={styles.cardImage} />
      <View style={[styles.statutDot, item.statut === 'autorise' ? styles.dotVert : styles.dotRouge]} />
      <View style={styles.cardInfo}>
        <Text style={styles.cardName} numberOfLines={1}>{item.name}</Text>
        <Text style={styles.cardRole}>{item.role}</Text>
        <Text style={styles.cardTime}>{item.date} · {item.heure}</Text>
        <View style={styles.cardBtns}>
          <TouchableOpacity style={[styles.cardBtn, item.statut === 'autorise' && styles.cardBtnActiveVert]} onPress={() => toggleStatut(item.id, 'autorise')}>
            <Text style={[styles.cardBtnText, item.statut === 'autorise' && { color: '#4ADE80' }]}>✓ Autorisé</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.cardBtn, item.statut === 'bloque' && styles.cardBtnActiveRouge]} onPress={() => toggleStatut(item.id, 'bloque')}>
            <Text style={[styles.cardBtnText, item.statut === 'bloque' && { color: '#F87171' }]}>✗ Bloqué</Text>
          </TouchableOpacity>
        </View>
      </View>
    </TouchableOpacity>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <View><Text style={styles.headerTitle}>Contr&ocirc;le d&apos;acc&egrave;s</Text><Text style={styles.headerSub}>ACC&Egrave;S &Agrave; LA MAISON</Text></View>
        </View>
        <TouchableOpacity style={styles.addBtn} onPress={() => router.push('/add-person')}><Text style={styles.addBtnText}>＋ Ajouter</Text></TouchableOpacity>
      </View>
      ...
      <View style={styles.statsRow}>
        ...
        <TouchableOpacity style={styles.statBox} onPress={() => setFilter('autorise')}>
          <Text style={[styles.statValue, { color: '#4ADE80' }]}>{autorise}</Text>
      ...
      <TouchableOpacity style={styles.banner} onPress={() => router.push('/surveillance')}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.bannerSub}>Les personnes autoris&eacute;es sont reconnues par la cam&eacute;ra</Text>
        </View>
      ...
      <View style={styles.searchWrapper}>
        ...
        <TextInput style={styles.searchInput} placeholder="Rechercher..." placeholderTextColor="#6B7A99" value={search} onChangeText={setSearch} />
            ...
            {selected && (
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.modalHeader}>
                  ...
                  <View style={[styles.modalBadge, selected.statut === 'autorise' ? styles.badgeVert : styles.badgeRouge]}>
                    <Text style={[styles.modalBadgeText, { color: selected.statut === 'autorise' ? '#4ADE80' : '#F87171' }]}>
                      {selected.statut === 'autorise' ? '✓ Autorisé' : '✗ Bloqué'}
                    </Text>
                  </View>
                </View>
                <View style={styles.modalInfoRow}><Text style={styles.modalInfoLabel}>Date détection</Text><Text style={styles.modalInfoValue}>{selected.date} à {selected.heure}</Text></View>
                <View style={styles.modalInfoRow}><Text style={styles.modalInfoLabel}>Rôle</Text><Text style={styles.modalInfoValue}>{selected.role}</Text></View>
                <Text style={styles.modalSectionTitle}>Modifier le statut</Text>
                <View style={styles.modalBtnRow}>
                  <TouchableOpacity style={[styles.modalBtn, selected.statut === 'autorise' && styles.modalBtnActiveVert]} onPress={() => toggleStatut(selected.id, 'autorise')}>
                    <Text style={styles.modalBtnIcon}>✅</Text>
                    <Text style={[styles.modalBtnText, selected.statut === 'autorise' && { color: '#4ADE80' }]}>Autoriser</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modalBtn, selected.statut === 'bloque' && styles.modalBtnActiveRouge]} onPress={() => toggleStatut(selected.id, 'bloque')}>
                    <Text style={styles.modalBtnIcon}>🚫</Text>
                    <Text style={[styles.modalBtnText, selected.statut === 'bloque' && { color: '#F87171' }]}>Bloquer</Text>
                  </TouchableOpacity>
                </View>
                <TouchableOpacity style={styles.modalInterphone} onPress={() => { setSelected(null); router.push('/interphone'); }}>
                  <Text style={styles.modalInterphoneIcon}>📞</Text>
                  <Text style={styles.modalInterphoneText}>Parler via interphone</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.modalDelete} onPress={() => deletePerson(selected.id)}>
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: '#7C3AED', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20 },
  ...
  banner: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#13132A', marginHorizontal: 14, marginTop: 12, borderRadius: 14, borderWidth: 1, borderColor: '#2E2B52', padding: 14 },
  bannerIconBox: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#2D1B6B', borderWidth: 1, borderColor: '#7C3AED', alignItems: 'center', justifyContent: 'center' },
  ...
  searchWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#13132A', borderWidth: 1, borderColor: '#2E2B52', borderRadius: 12, marginHorizontal: 14, marginTop: 12, paddingHorizontal: 14 },
  ...
  filterChip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: '#13132A', borderWidth: 1, borderColor: '#2E2B52' },
  ...
  card: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#13132A', borderRadius: 14, borderWidth: 1, borderColor: '#2E2B52', marginBottom: 10, padding: 12, gap: 12, position: 'relative' },
  cardBloque: { borderColor: '#4B1515', backgroundColor: '#150E0E' },
  cardImage: { width: 58, height: 58, borderRadius: 12 },
  statutDot: { position: 'absolute', top: 10, left: 58, width: 12, height: 12, borderRadius: 6, borderWidth: 2, borderColor: '#0D0D1A' },
  ...
  cardBtn: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, backgroundColor: '#1A1A2E', borderWidth: 1, borderColor: '#2E2B52' },
  ...
  fab: { position: 'absolute', bottom: 90, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: '#7C3AED', alignItems: 'center', justifyContent: 'center', shadowColor: '#7C3AED', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 8 },
  fabText: { color: '#FFFFFF', fontSize: 28, fontWeight: 'bold', marginTop: -2 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' },
  modalSheet: { backgroundColor: '#13132A', borderTopLeftRadius: 24, borderTopRightRadius: 24, borderWidth: 1, borderColor: '#2E2B52', paddingHorizontal: 20, paddingBottom: 36, maxHeight: '85%' },
  modalBar: { width: 40, height: 4, borderRadius: 2, backgroundColor: '#2E2B52', alignSelf: 'center', marginTop: 12, marginBottom: 20 },
  modalHeader: { alignItems: 'center', marginBottom: 20 },
  modalImage: { width: 90, height: 90, borderRadius: 45, borderWidth: 3, borderColor: '#7C3AED', marginBottom: 12 },
  modalName: { fontSize: 20, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 4 },
  modalRole: { fontSize: 13, color: '#A78BFA', marginBottom: 10, fontWeight: '600' },
  ...
  modalInfoRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#2E2B52' },
  modalInfoLabel: { fontSize: 13, color: '#8A8FAB' },
  modalInfoValue: { fontSize: 13, color: '#FFFFFF', fontWeight: '600' },
  modalSectionTitle: { fontSize: 13, color: '#8A8FAB', fontWeight: '700', marginTop: 16, marginBottom: 10, letterSpacing: 0.6 },
  modalBtnRow: { flexDirection: 'row', gap: 10, marginBottom: 14 },
  modalBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 12, borderRadius: 12, backgroundColor: '#1A1A2E', borderWidth: 1, borderColor: '#2E2B52' },
  ...
  modalInterphone: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#1E1040', borderWidth: 1, borderColor: '#7C3AED', borderRadius: 12, paddingVertical: 13, marginBottom: 10 },
  ...
  modalClose: { backgroundColor: '#1A1A2E', borderWidth: 1, borderColor: '#2E2B52', borderRadius: 12, paddingVertical: 13, alignItems: 'center' },

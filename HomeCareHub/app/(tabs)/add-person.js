...
export default function AddPerson() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.1.108:8000/api/personnes/ajouter/', {
        method: 'POST',
        body: formData,
      });
...
const styles = StyleSheet.create({
  ...
  addBtn:           { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#7C3AED', paddingVertical: 16, borderRadius: 14, marginTop: 8, shadowColor: '#7C3AED', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 10, elevation: 5 },
  addBtnIcon:       { fontSize: 18 },
  addBtnText:       { color: '#FFFFFF', fontSize: 17, fontWeight: 'bold', letterSpacing: 0.4 },
});